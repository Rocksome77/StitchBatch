...
// Running this script multiple times on the same data set will result in the images moving out of their proper alignment as they get shuffled down one space and the new Image #1 is inserted into the central well.

directory = getDirectory("Input directory"); // Identify folder containing the individual source images
File.makeDirectory(directory+"stitched/") // Creates a subdirectory of the renamed folder to store the stitched images
File.makeDirectory(directory+"stitched/overlayed/") // Creates a subdirectory of the stitched folder to store the images with multiple channels overlayed
		...
		if (x < 10) // Create string from column number (x co-ordinate) used for loading/saving files and adding 1 or 2 leading zeroes depending on number of digits
			...
			for (c=1; c <= 4; c++) // Begin working through channels: lowest to highest
				...
				wellCoord = yCo + xCo + cCo;
			  	stitch(directory, directory +"stitched/", genericFilename, wellCoord);
				}
			filename = yCo + xCo; // Generates partial new filename for overlayed images 
    		overlay(directory +"stitched/", directory +"stitched/overlayed", filename); // overlay calling overlay function below
		}
	}
...
function renameimages(xCo, yCo, cCo, centralWell) {
	for (f=1; f <=centralWell; f++)    // Begin working through each FOV: lowest to the central well
		...
		File.rename(directory + oldName, directory +"renamed/"+newName); // shift all image before the central well down 1 step and into the "renamed" folder
		}
	...
	File.rename(directory + oldName, directory+newName);
	for (f=centralWell+1; f <=FOVwidth * FOVheight; f++)
		...
		File.rename(directory + oldName, directory+oldName);		
		}
	}
