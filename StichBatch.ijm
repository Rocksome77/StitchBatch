...
function renameimages(xCo, yCo, cCo, centralWell) {
	for (f=1; f <=centralWell; f++)    // Begin working through each FOV: lowest to the central well
		{
		f2 = f--; // Shift all images down one step
	...
	for (f=centralWell+1; f <=FOVwidth*FOVheight; f++)
		{
   		oldName = yCo + xCo +"-"+f+"-001001"zCo+".tif"; // For the remained for the wells keep filenames the same but move to the renamed folder 
...
function overlay(stitched, overlayed, filename) {
		...
		run("Yellow"); 			
		run("Merge Channels...", c2="+filename+"002.tif c7="+filename+"004.tif create");
